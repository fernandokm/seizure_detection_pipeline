...
def compute_metrics(
    ...
    try:
        titles_options = [
            (f"{prefix} - Confusion Matrix", None),
        ...
        for title, normalize in titles_options:
            ...
            disp = ConfusionMatrixDisplay(
                confusion_matrix=cm_disp, display_labels=[0, 1]
            )
...
def clean_ml_dataset(df_ml: pd.DataFrame, target_treshold: float = 0.5) -> pd.DataFrame:
    """
    ...
    df_ml["label"] = df_ml["label"].apply(lambda x: 1 if x >= target_treshold else 0)
...
def parse_train_model_args(args_to_parse: List[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--ml-dataset-path", dest="ml_dataset_path", required=True)
