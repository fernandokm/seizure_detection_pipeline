...
def write_database(
    export_folder: str, df_data: pd.DataFrame, df_annotations: pd.DataFrame
) -> Tuple[str, str, str]:
    ...
    df_candidates = df_data.merge(
        df_annotations, how="outer", on=["exam_id", "patient_id"]
    )
    ...
    # Remove non NA
    df_candidates["annotations_file_path"] = df_candidates["edf_file_path"].apply(
        lambda x: parse_tse_bi(x, db=DB)
    )
...
def parse_tse_bi(edf_file_path, db: str = "tuh"):
    try:
        if db == "tuh":
            split_limit = re.search("[.][e][d][f]", edf_file_path).start()
            tse_bi_file_path = ".".join([edf_file_path[:split_limit], "tse_bi"])
            ...
            split_limit = re.search("[_][s]\w*[.][e][d][f]", edf_file_name).start()
            tse_bi_file_name = ".".join([edf_file_name[:split_limit], "tse_bi"])
            tse_bi_file_path = "".join(
                [tse_bi_folder_path, "/", patient_id, "_Annotations_", tse_bi_file_name]
            )
...
def fetch_database(
    ...
    df_data = pd.DataFrame(columns=["data_file_path", "exam_id", "patient_id"])

    data_call = subprocess.Popen(
        ("find", "-L", data_folder_path, "-type", "f", "-iname", data_file_pattern),
    ...
    df_annotations = pd.DataFrame(
        columns=["data_file_path", "exam_id", "patient_id", "annotator_id"]
    )
    annotations_call = subprocess.Popen(
        (
            "find",
            "-L",
            data_folder_path,
    ...
    data_path, annotation_path, candidates_path = write_database(
        export_folder, df_data, df_annotations
    )
...
def parse_fetch_database_args(args_to_parse: List[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--data-folder-path", dest="data_folder_path", required=True)
    parser.add_argument("--export-folder", dest="export_folder")
    parser.add_argument("--data-file-pattern", dest="data_file_pattern")
    parser.add_argument("--patient-pattern", dest="patient_pattern")
    parser.add_argument("--exam-pattern", dest="exam_pattern")
    parser.add_argument("--annotations-file-pattern", dest="annotations_file_pattern")
