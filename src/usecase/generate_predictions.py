"""
This script computes predictions on a patient.

It loads a csv file and a pickled model, generates the predictions,
...
def generate_predictions(models: dict,
    ...
    """
    Generates predictions using the specified models.

    This function returns a new dataframe, which contains the same data
    as df, as well as two additional columns:

        predicted_label : int
            the predictions generated by the model
        model : str
            the model name (the key in the models dictionary)

    If there are multiple models, the rows of df will be repeated
    (once per model).
    ...
    df : pd.DataFrame
        The dataframe on which to apply the model.

    Returns
    -------
    pd.DataFrame
        A new dataframe which indicates the predictions generated by each
        model.
    """
    df_models = []
...
def generate_predictions_cli(model_path: str,
    ...
    This function is a wrapper of generate_predictions which enables
...
def parse_generate_prediction_args(
    ...
    parser.add_argument('--output-folder', default='output/preds-v0_6',
                        dest='output_folder',
