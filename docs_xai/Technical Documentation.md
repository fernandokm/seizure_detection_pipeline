...
Otherwise, using a diagnostic tool based on artificial intelligence can generate mistrust among **doctors** because these algorithms are generally difficult to understand at first glance due to their complexity. However, doctors must absolutely have confidence in these algorithms since they are responsible for letting their patients use the tool. To generate this confidence in the solution, it is therefore important that doctors understand the underlying mechanisms of the algorithm.
...
**Doctors** : The second stakeholder is the doctors team, we need to provide them with an understandable and transparent solution to generate confidence in the solution.
...
**Shap :** We used _Shap.TreeExplainer_ to explain the output of our tree model, and therefore in the case where the ML model needs to be modified we will distinguish between two cases:

- **_if it is a The tree-based machine learning model :_**

You do not have to change anything in the code because [_Shap.TreeExplainer_](https://shap-lrjball.readthedocs.io/en/latest/generated/shap.TreeExplainer.html") support XGBoost, LightGBM, CatBoost, Pyspark and most of the scikit-learn tree models.

- **_if it's another type :_**
...
- _Data Scientist :_ in order to customize the 'data scientist layout' according to the expectations of this user profile. We have decided to put graphs and figures that give information about the performance and functioning of the prediction algorithm.

- _Doctors :_ The 'doctor layout' will contain the data that makes the most sense to doctors for epilepsy prediction as well as the graphs that show the features that the model relied on to make such a decision so that they can see if the algorithm is using the right criteria in its reasoning.
...
> 1. _Evolution of the feature as a function of time._ > ![modified_csi in function of time ](imgs/grapheFeature.jpeg)
...
- **_Patient sheet:_** This sheet will allow them to identify the patient and will contain information such as ( Name, First name, Age, sex..).
