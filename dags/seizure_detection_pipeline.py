...
def dag_seizure_detection_pipeline():
    ...
    def t_detect_qrs(parameters_list: List[dict]) -> List[dict]:
        ...
        def inner(parameters: dict):
            ...
            output_parameters = {
                "rr_file_path": output_qrs_file_path,
    ...
    def t_compute_hrv_analysis_features(parameters_list: List[dict]) -> List[dict]:
        ...
        def inner(parameters: dict) -> dict:
            ...
            output_parameters = {"features_file_path": output_features_file_path}
    ...
    @task()
    def t_compute_consolidate_feats_and_annot(
        ...
        def inner(parameters: dict) -> dict:
            ...
            output_parameters = {"cons_file_path": output_cons_file_path}
    ...
    def t_get_initial_parameters(partition_index: int) -> List[dict]:
        ...
        df_db = pd.read_csv(
            f"{FETCHED_DATA_FOLDER}/df_candidates.csv", encoding="utf-8"
        )
        ...
        for index in range(df_db.shape[0]):
            ...
            parameters = {
                "qrs_file_path": "".join([AIRFLOW_PREFIX_TO_DATA, qrs_file_path]),
                "annotations_file_path": "".join(
                    [AIRFLOW_PREFIX_TO_DATA, tse_bi_file_path]
                ),
                "exam_id": exam_id,
    ...
    for partition_index in range(NUM_PARTITIONS):
        ...
        parameters_list = t_consolidate_parameters(parameters_list, qrs_parameters_list)

        features_parameters_list = t_compute_hrv_analysis_features(parameters_list)
        parameters_list = t_consolidate_parameters(
            parameters_list, features_parameters_list
        )
        ...
        parameters_list = t_consolidate_parameters(
            parameters_list, consolidated_feats_and_annot_parameters_list
        )
...
def dag_model_pipeline():
    ...
    def t_get_initial_parameters(partition_index: int) -> List[dict]:
        ...
        for index in range(df_db.shape[0]):
            ...
            parameters = {
                "cons_file_path": path.join(CONSOLIDATED_FOLDER, cons_file_path),
                "consolidated_folder": CONSOLIDATED_FOLDER,
    ...
    def t_generate_predictions_and_shap(parameters_list: List[dict]) -> List[dict]:
        ...
        def inner(parameters: dict) -> dict:
            ...
            return {"predictions_file_path": predictions_file_path}
...
def dag_grafana_pipeline():
    ...
    def t_get_initial_parameters() -> dict:
        ...
        preds_files = [
            os.path.join(PREDICTIONS_FOLDER, file)
    ...
    def t_generate_dashboards(parameters: dict) -> None:
        from src.visualizations.generate_dashboards import (
    ...
    t_generate_dashboards(parameters)
